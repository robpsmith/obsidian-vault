...
# Required env vars (set by start-gitlab.sh or export manually):
#   GITLAB_HOME  — host path for GitLab data  (default: /etc/gitlab)
#   IP_ADDRESS   — server IP to bind port 443  (auto-detected by start-gitlab.sh)
services:
  gitlab:
...
# Uses restart: "no" so the container won't loop if reconfigure fails.
#
# All configuration lives in gitlab.rb — no GITLAB_OMNIBUS_CONFIG overrides.
# To block CI runners during a restore, add this line to gitlab.rb BEFORE starting:
#   nginx['custom_gitlab_server_config'] = "location ^~ /api/v4/jobs/request { return 503; }"
# Remove that line after switching to the production compose.
services:
  gitlab:
    image: gitlab/gitlab-ee:18.8.2-ee.0
    container_name: gitlab
